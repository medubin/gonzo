...
func (rtr *Router) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, e := range rtr.routes {
		...
		if hasMiddleware {
			...
			rtr.handleMiddlewareResponse(w, ctx, middlewareReq, responseCapture, routeInfo)
		} else {
...
func (rtr *Router) handleMiddlewareResponse(w http.ResponseWriter, ctx context.Context, req *middleware.MiddlewareRequest, responseCapture *responseWriter, info *middleware.RouteInfo) {
	...
	var err error
	for _, m := range rtr.middleware {
		middlewareResp, err = m.AfterHandler(ctx, req, middlewareResp, info)
		if err != nil {
