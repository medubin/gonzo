package server

import (
	"context"
	"github.com/medubin/gonzo/runtime/cookies"
	"github.com/medubin/gonzo/runtime/gerrors"
	"github.com/medubin/gonzo/runtime/handle"
	"github.com/medubin/gonzo/runtime/url"
)

// GET /users/{userId}/notifications
func (s *NotificationServiceImpl) GetUserNotifications(ctx context.Context, body *struct{}, cookie cookies.Cookies, url url.URL[UserListParams, GetUserNotificationsUrl]) (*handle.Response[[]Notification], error) {
  return nil, gerrors.UnimplementedError("GetUserNotifications")
}
